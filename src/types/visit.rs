...
pub trait Visit<'ctxt> {
    fn super_visit_type(&mut self, ty: Type<'ctxt>) {
        match ty.kind() {
            ...
            &(TypeKind::Array(ty) | TypeKind::Box(ty) | TypeKind::RawDynArray(ty)) => self.visit_type(ty),
...
pub trait VisitMut<'ctxt> {
    ...
    fn super_visit_type(&mut self, ty: Type<'ctxt>) -> Type<'ctxt> {
        match ty.kind() {
            ...
            &TypeKind::RawDynArray(ty) => Type::new_raw_dyn_array(self.ctxt(), self.visit_type(ty)),
