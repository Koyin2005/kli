use crate::{typecheck::infer::TypeInfer, typed_ast::{Expr, ExprKind, Pattern, PatternKind, Place, PlaceKind}, types::{FunctionType, GenericArg, Region, Type}};
...
impl<'a> TypeSubst<'a>{
    ...
    pub fn subst_type(&mut self, ty: &mut Type){
        match ty{
            Type::Bool | Type::Int | Type::String | Type::Unit | Type::Unknown | Type::Param(..) => (),
    ...
    pub fn subst_expr(&mut self, expr: &mut Expr){
        match &mut expr.kind{
            ExprKind::Bool(_) | ExprKind::Err | ExprKind::Unit | ExprKind::Int(_) | ExprKind::String(_) | ExprKind::Panic | ExprKind::None => (),
            ExprKind::Sequence(first,second) | ExprKind::Binary(_,first,second) => {
                self.subst_expr(first);
                self.subst_expr(second);
            },
            ExprKind::Print(expr) => if let Some(expr) = expr{
                self.subst_expr(expr);
            },
            ExprKind::Some(expr) => self.subst_expr(expr),
            ExprKind::List(exprs) => for expr in exprs{
                self.subst_expr(expr);
            },
            ...
            ExprKind::Let { pattern, binder, body } => {
                self.subst_pattern(pattern);
